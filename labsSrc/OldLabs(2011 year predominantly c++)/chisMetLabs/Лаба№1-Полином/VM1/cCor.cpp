...
void Interpolation::DrawAxes()
{
	...
	{
		// на вертикальной оси
		IBackground->Canvas->MoveTo( 2 * SPACE - 10 - 5,
...
double Interpolation::F(double x)
{
	...
	else
	{
		...
		// считаем окончательный результат
        if((c_Gamma != x) || (c_Betta == 0))
                rez = c_Alpha * sin(c_Betta/(x - c_Gamma)/(x - c_Gamma));
        if((c_Mu != x) || (c_Eps == 0))
                rez +=c_Delta * cos(c_Eps/(x - c_Mu)/(x - c_Mu));
        if((c_Mu == x) && (c_Gamma == x) && (c_Betta!=0) && (c_Eps!=0))
                rez=1001;
	//}
...
void Interpolation::Print_F()
{
	...
	IGraphs->Canvas->Pen->Color = MainForm->F_X->Selected;
	IGraphs->Canvas->Pen->Style = psSolid;
...
void Interpolation::Print_DF()
{
	...
	// сплошное перо одинарной толщины
	IGraphs->Canvas->Pen->Color = MainForm->dF_X->Selected;
	IGraphs->Canvas->Pen->Style = psSolid;
	IGraphs->Canvas->Pen->Width = 1;
...
void Interpolation::Print_P()
{
	...
	// сплошное перо одинарной толщины
	IGraphs->Canvas->Pen->Color = MainForm->P_X->Selected;
	IGraphs->Canvas->Pen->Style = psSolid;
	IGraphs->Canvas->Pen->Width = 1;
	...
	for( p = c_A; p <= c_B; p += ( c_B - c_A ) / ( c_N - 1 ) )
		Point(MainForm->P_X->Selected, ( p - c_A ) * ( IGraphs->Width - 1 ) / ( c_B - c_A ), IGraphs->Height * ( F( p ) - c_D ) / ( c - c_D ) );
...
void Interpolation::Print_DP()
{
	...
	// сплошное перо одинарной толщины
	IGraphs->Canvas->Pen->Color = MainForm->dP_X->Selected;
	IGraphs->Canvas->Pen->Style = psSolid;
	IGraphs->Canvas->Pen->Width = 1;
...
void Interpolation::Print_R()
{
	...
	// сплошное перо одинарной толщины
	IGraphs->Canvas->Pen->Color = MainForm->R_X->Selected;
	IGraphs->Canvas->Pen->Style = psSolid;
	IGraphs->Canvas->Pen->Width = 1;
...
void Interpolation::Update()
{
        ...
        FunctionString += "/(x - ";
        FunctionString += MainForm->Edit2->Text;
        FunctionString += ")^2";
        ...
        SetMu(MainForm->Edit2->Text.ToDouble());
