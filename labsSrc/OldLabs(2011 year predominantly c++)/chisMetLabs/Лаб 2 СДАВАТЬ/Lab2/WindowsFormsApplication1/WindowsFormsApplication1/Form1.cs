...
namespace WindowsFormsApplication1
{
    ...
    {
        double A = -1, B = 1, C = -1, D = 1, Gamma, Beta, Alpha, Delta, a, b, sh, n_max;
        int n;
        ...
        double Integral(double p1, int tmp)
        {
            ...
            if (radioButton_Alph.Checked)
            {
                ...
                {
                    InSum += (p1 * Math.Cos(Math.Tan(Beta * x)) * Math.Cos(Gamma * x) + p1 * Math.Cos(Math.Tan(Beta * (x+h))) * Math.Cos(Gamma * (x+h)));
                }
                return (h / 2.0) * InSum;
            }
            ...
            if (radioButton_Bet.Checked)
            {
                for (double x = a; x < b; x += h)
                {
                    InSum += (Alpha * Math.Cos(Math.Tan(p1 * x)) * Math.Cos(Gamma * x) + Alpha * Math.Cos(Math.Tan(p1 * (x + h))) * Math.Cos(Gamma * (x + h)));
                }
                return (h / 2.0) * InSum;
            }
            ...
            if (radioButton_Gamma.Checked)
            {
                for (double x = a; x < b; x += h)
                {
                    InSum += (Alpha * Math.Cos(Math.Tan(Beta * x)) * Math.Cos(p1 * x) + Alpha * Math.Cos(Math.Tan(Beta * (x + h))) * Math.Cos(p1 * (x + h))); ;
                }
                return (h / 2.0) * InSum;
            }
        ...
        private void Form1_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            {
                About foto = new About();
                foto.StartPosition = FormStartPosition.CenterParent;
                foto.ShowDialog(this);
            }
        }
    }
