...
#pragma resource "*.dfm"
TMainForm *MainForm;
long double x,y,Dots[2][50],alpha=1,beta=2,gamma=3,delta=4,epsilon=5,A=-2,B=2,C=-2,D=2,H=0.01,x02,y02;
...
{if(Sender==N11)
 ...
 if(Sender==N13)
  MainForm->Close();
 if(Sender==PanelTask||Sender==LTaskTitle)
 ...
 if(Sender==PanelAbout||Sender==LAboutTitle||Sender==MemoAbout||
    Sender==PanelPhoto||Sender==ImagePhoto)
 ...
 {{double test;
   ...
   if(!TryStrToFloat(EditEpsilon->Text,test)||test<-100||test>100)
   {Application->MessageBoxA("Неверное значение параметра Эпсилон","Внимание!",MB_ICONWARNING);
    EditEpsilon->SetFocus();
    return;
   }
...
{int i;
 ...
 {if(x>1000||x<-1000||y>1000||y<-1000)
   break;
  /*if((stepen=powl(2,y))>10e7) break;
  x+=(alpha*tanl(beta*y-gamma*x))*H;
  y+=(delta*stepen-2*cosl(M_PI/3-epsilon*x2))*H;*/
  /*temp=powl(2.7,H*i);
  x+=(y+2*temp)*H;
  temp=powl(H*i,2);
  y+=(x2+temp)*H;*/
  /*x+=(2*x*y-alpha*x+beta*y)*H;
  y+=(5*powl(x2,4)+gamma*powl(y,3)+2*delta*x2-3*epsilon*y)*H;*/
  x+=(5*alpha*x-3*beta*y+2*gamma)*H;
  y+=(delta*x2+epsilon*y+5)*H;
  if(error==1) break;
