...
namespace laba_1
{
    public partial class Form1 : Form
    {
        ...
        void Pn()
        {
            ...
            double[] x_arr = new double[n]; // TMP
            double[] y_arr = new double[n]; // TMP
            ...
            for (double x = A; x <= B; x+=(B-A)/10)
            {
                ...
                //y = stirling(x, x_arr, y_arr, n, h); // TMP
        ...
        // TEMP
        double delta(Int64 i, Int64 n, double[] y)
        {
            if (n == 1)
                return (y[i + 1] - y[i]);
            else
                return (delta(i + 1, n - 1, y) - delta(i, n - 1, y));
        }
        // TEMP
        double stirling(double vx, double[] x, double[] y, Int64 n, double h)
        {
            Int64 i, j, k = 1, m, l;
            double vy = 0, p, q;

            m = 0;
            k = Convert.ToInt32( Math.Abs(vx - x[0]) );

            for(i = 1; i < n; i++)
                if(k > Math.Abs(vx - x[i]))
                    m = i;
            ...
            for(i=1;i<(l-1);i++)
            {
                 ...
                 if(i%2==0)
                 {
                    vy = vy + (q * delta(m-(i/2), i, y) );
                 }
                 else
                 {
                    vy=vy+(q*((delta(m-(i/2),i,y)+delta(m-(i/2)-1,i,y))/2));
                 }
            }
