...
namespace WindowsFormsApplication1
{
    ...
    {
        double A = -1, B = 1, C = -1, D = 1, y, Gam, Bet, Alph, Dlt, Delt, sh, h, t, tmp = 1, x0;
        int n, e = 1;
        ...
        private void Build_up_Click(object sender, EventArgs e)
        {
            ...
            Dlt = Convert.ToDouble(this.Delta1.Value);
        ...
        void fx()//график исходной функции
        {
            ...
            {
                y = Alph * (Math.Cos(Math.Tan(Bet * x))) + Gam * (Math.Cos(Dlt * x));
        ...
        {
            double xx = (A + B) / 2;
            double p = Alph * (Math.Cos(Math.Tan(Bet * xx))) + Gam * (Math.Cos(Dlt * xx));
        ...
        void InicialPolinom()//инициализация полинома и расчёт таблицы центральных разностей
        {
            ...
            {
                rez[a, 0] = Alph * (Math.Cos(Math.Tan(Bet * x))) + Gam * (Math.Cos(Dlt * x));
        ...
        void Rnx()
        {
            ...
            for (double x = A; x <= B; x += sh)//вычисляем точки функции с указанным шагом
            {
                    ...
                    tmp = 1;
                    e = 1;
                    y = Math.Abs(Alph * (Math.Cos(Math.Tan(Bet * x))) + Gam * (Math.Cos(Dlt * x)) - Polinom(n, t));
